...
do
	...
	if [[ `echo "$disk_smart" | grep 'Unknown USB bridge'` ]]; then
        ...
        disk_name=`echo $series$prettycount'_'$disk_model'_'$disk_capacity'_'$disk_serial`
		disk_name=`echo $disk_name | sed 's/__/_/'`
	
	else
	    
        disk_capacity=`echo "$disk_smart" | grep 'User Capacity:' | sed 's/.*\://g' | grep -Eo '.*bytes' | tr -d "[A-Za-z], \t"`
        disk_capacity="$(( $disk_capacity / (1024*1024*1024) ))GB"

        disk_model=`echo "$disk_smart" | grep -E 'Device Model:|Product:' | sed 's/.*\://g' | tr -d " \t"`
        ...
        disk_name=`echo $series$prettycount'_'$disk_vendor'_'$disk_model'_'$disk_capacity'_'$disk_serial`
	    disk_name=`echo $disk_name | sed 's/__/_/'`
    ...
    do
        echo $disk_partition | grep -Eo '.*part[0-9]'
    done
